...
Pressing the up-button starts the 60-second countdown, measuring the interval from the beginning of the timeout to the ball has to be declared ready for play (the remaining 25 seconds are job of the playclock operator), giving a vibration signal as confirmation.  
Pressing the up-button long works in the same manner except the countdown starts at 30 seconds to measure a 30-second timeout.  
...
Starts 60" timeout countdown (new state: *TIMEOUT RUNNING*)
#### UP-Button pressed long (*2x short vibes*)
Starts 30" timeout countdown (new state: *30" TO RUNNING*)
#### SELECT-Button
Disabled - no action
...
#### SELECT-Button pressed long
Stops 60" timeout countdown (new state: *TO STOPPED*)
...
The 30 second timeout countdown works the same way as described for the 60 second timeout countdown.

![30sec TO RUNNING](doc/TOshort_running.png "30sec TO RUNNING")

### 1' IM RUNNING / 1' IM EXPIRED / 1' IM STOPPED
The 1 minute countdown works the same way as described for the 60 second timeout countdown.
...
Den oberen Knopf zu drücken statet den 60-Sekunden Countdown, um das Intervall vom Beginn des Timeouts bis zur Ballfreigabe zu messen (die übrigen 25 Sekunden sind Aufgabe des PlayClock-Verantwortlichen), was mit einem Vibrationssignal bestätigt wird.  
Den oberen Knopf länger zu drücken führt zu dem selben Ergebnis, allerdings startet der Countdown bei 30 Sekunden, um ein 30-Sekunden-Timeout zu messen.  
...
Startet den 60" Timeout Countdown (neuer Zustand: *TIMEOUT RUNNING*)
#### Knopf oben lange gedrückt (*2x kurze Vibrationen*)
Startet den 30" Timeout Countdown (neuer Zustand: *30" TO RUNNING*)
#### Knopf in der Mitte
...
![TIMEOUT RUNNING](doc/TO_running.png "TIMEOUT RUNNING")
#### Knopf in der Mitte lange gedrückt
Stoppt den 60" Timeout Countdown (neuer Status: *TO STOPPED*)
...
### 30" TO RUNNING / 30" TO EXPIRED / 30" TO STOPPED
Der 30-Sekunden Countdown funktioniert gleichermaßen wie für den 60-Sekunden Timeout Countdown beschrieben.

![30sec TO RUNNING](doc/TOshort_running.png "30sec TO RUNNING")

### 1' IM RUNNING / 1' IM EXPIRED / 1' IM STOPPED
Der 1-Minuten Countdown funktioniert gleichermaßen wie für den 60-Sekunden Timeout Countdown beschrieben.
